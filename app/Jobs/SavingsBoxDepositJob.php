...
class SavingsBoxDepositJob implements ShouldQueue
{
    ...
    public function handle(DashboardRealtimePayloadBuilder $payloadBuilder): void
    {
        ...
        DB::transaction(function () use (&$shouldBroadcast): void {
            ...
            if (! $transaction || ! $wallet || ! $savingsBox) {
                Log::error('savings_box.deposit_failed', [
            ...
            if ($walletBalanceCents < (int) $this->amountCents) {
                Log::error('savings_box.deposit_failed', [
                    'transaction_id' => $transaction->id,
                    'savings_box_id' => $savingsBox->id,
                    'amount_cents' => $this->amountCents,
                    'reason' => 'Insufficient wallet balance in job execution',
                ]);
            ...
            $newWalletBalanceCents = $walletBalanceCents - (int) $this->amountCents;
        ...
        if ($shouldBroadcast) {
            $payload = $payloadBuilder->buildForUser($this->userId, 'savings_box_deposit_completed', [$this->transactionId]);
